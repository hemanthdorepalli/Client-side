...
const MySession: React.FC<MySessionProps> = ({ user }) => {
  ...
  useEffect(() => {
    const fetchSlots = async () => {
      try {
        const response = await axios.get('http://localhost:5021/api/availability/slots', {
  ...
  const handleDelete = async (_id: string) => {
    try {
      await axios.delete(`http://localhost:5021/api/availability/slots/${_id}`);
  ...
  const handleEditSave = async () => {
    if (editSlot) {
      try {
        await axios.put(`http://localhost:5021/api/availability/slots/${editSlot._id}`, {
